...
// Local and hosted launchers can inherit stale environment variables. The
// project .env is the authoritative source for this API process.
...
app.get("/ready", async (_req, res) => {
  try { await prisma.$queryRaw`SELECT 1`; res.json({ status: "ready" }); }
    ...
    const rawMessage = typeof databaseError.message === "string" ? databaseError.message : "No error message";
    const safeMessage = rawMessage.replace(/postgres(?:ql)?:\/\/\S+/gi, "[REDACTED_DATABASE_URL]");
...
apiRouter.use("/branches", networkRoutes);
