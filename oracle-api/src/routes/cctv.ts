...
const audit = (req: AuthRequest, action: string, entity: string, entityId: string, metadata: Record<string, unknown>) => prisma.activityLog.create({ data: { userId: req.user!.id, action, entity, entityId, metadata: { source: "cctv_manual", actorId: req.user!.id, ...metadata } } });
...
router.post("/cameras", requirePermission("manage_infrastructure_assets"), async (req: AuthRequest, res) => {
  const b = req.body as Record<string, unknown>; const allowed = ["assetId", "physicalLocation", "coverageArea", "cameraType", "resolution", "nightVision", "motionDetection", "installationDate", "notes", "networkInterfaceId"];
  ...
  const networkInterfaceId = typeof b.networkInterfaceId === "string" ? b.networkInterfaceId : null; if (networkInterfaceId) { const iface = await prisma.networkInterface.findUnique({ where: { id: networkInterfaceId }, select: { asset: { select: { branchId: true } } } }); if (!iface || iface.asset.branchId !== s.asset.branchId) return res.status(400).json({ error: "Network interface must belong to the camera branch.", code: "INVALID_NETWORK_REFERENCE" }); }
  const data = { assetId, physicalLocation: location, coverageArea: typeof b.coverageArea === "string" ? b.coverageArea.trim() : null, cameraType: (b.cameraType as never) ?? "fixed", resolution: typeof b.resolution === "string" ? b.resolution.trim() : null, nightVision: typeof b.nightVision === "boolean" ? b.nightVision : null, motionDetection: typeof b.motionDetection === "boolean" ? b.motionDetection : null, installationDate: typeof b.installationDate === "string" ? new Date(b.installationDate) : null, notes: typeof b.notes === "string" ? b.notes.trim() : null, networkInterfaceId };
...
router.post("/recorders", requirePermission("manage_infrastructure_assets"), async (req: AuthRequest, res) => {
  ...
  const channels = b.channels === undefined ? [] : b.channels; if (!Array.isArray(channels) || channels.some((c) => !c || typeof c !== "object" || Array.isArray(c) || unknown(c as Record<string, unknown>, ["channelNumber", "label", "enabled", "portId"]).length || !Number.isInteger((c as Record<string, unknown>).channelNumber))) return res.status(400).json({ error: "channels must contain valid channel rows.", code: "INVALID_CHANNELS" });
  ...
  const portIds = (channels as Record<string, unknown>[]).map((c) => typeof c.portId === "string" ? c.portId : null).filter((id): id is string => Boolean(id));
  if (portIds.length) { const ports = await prisma.networkPort.findMany({ where: { id: { in: portIds } }, select: { id: true, asset: { select: { branchId: true } } } }); if (ports.length !== portIds.length || ports.some((port) => port.asset.branchId !== s.asset!.branchId)) return res.status(400).json({ error: "Channel port references must belong to the recorder branch.", code: "INVALID_PORT_REFERENCE" }); }
