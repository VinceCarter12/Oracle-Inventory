...
router.post("/items", requirePermission("manage_stock"), async (req: AuthRequest, res) => { if (!(await enabled())) return gate(res); const b = req.body as Record<string, unknown>; const bad = unknown(b, ["sku", "name", "category", "unitOfMeasure", "description", "isSerialized"]); if (bad.length || rejectSecret(b) || typeof b.sku !== "string" || typeof b.name !== "string" || typeof b.category !== "string" || typeof b.unitOfMeasure !== "string") return res.status(400).json({ error: "Valid stock item fields are required; secrets are rejected.", code: "INVALID_STOCK_ITEM", fieldErrors: bad }); try { const item = await prisma.stockItem.create({ data: { sku: b.sku.trim().toUpperCase(), name: b.name.trim(), category: b.category.trim(), unitOfMeasure: b.unitOfMeasure.trim(), description: typeof b.description === "string" ? b.description.trim() : null, isSerialized: b.isSerialized === true } }); return res.status(201).json(item); } catch { return res.status(409).json({ error: "SKU already exists.", code: "DUPLICATE_SKU" }); } });
router.get("/items", requirePermission("view_stock"), async (req: AuthRequest, res) => { if (!(await enabled())) return gate(res); const q = typeof req.query.q === "string" ? req.query.q : undefined; const page = Math.max(1, Number(req.query.page) || 1); const pageSize = Math.min(100, Math.max(1, Number(req.query.pageSize) || 25)); const where = q ? { active: true, OR: [{ sku: { contains: q, mode: "insensitive" as const } }, { name: { contains: q, mode: "insensitive" as const } }] } : { active: true }; const [items, total] = await prisma.$transaction([prisma.stockItem.findMany({ where, skip: (page - 1) * pageSize, take: pageSize, orderBy: { name: "asc" } }), prisma.stockItem.count({ where })]); const enriched = await Promise.all(items.map(async (item) => ({ ...item, balance: await balance(item.id) }))); return res.json({ items: enriched, page, pageSize, total }); });
router.post("/locations", requirePermission("manage_stock"), async (req: AuthRequest, res) => { if (!(await enabled())) return gate(res); const b = req.body as Record<string, unknown>; const bad = unknown(b, ["branchId", "name", "locationType", "description"]); if (bad.length || rejectSecret(b) || typeof b.branchId !== "string" || typeof b.name !== "string") return res.status(400).json({ error: "Valid location fields are required.", code: "INVALID_STOCK_LOCATION", fieldErrors: bad }); const location = await prisma.stockLocation.create({ data: { branchId: b.branchId, name: b.name.trim(), locationType: b.locationType as never, description: typeof b.description === "string" ? b.description.trim() : null } }); return res.status(201).json(location); });
router.post("/movements", requirePermission("manage_stock"), async (req: AuthRequest, res, next) => {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (prior) {
        const same = prior.stockItemId === stockItemId && prior.movementType === movementType && Number(prior.quantity) === quantity && prior.entries.some((entry) => entry.locationId === locationId);
      ...
      const movement = await tx.stockMovement.create({ data: { stockItemId, movementType: movementType as never, quantity, reason: typeof b.reason === "string" ? b.reason.trim() : null, idempotencyKey: key, performedById: req.user!.id } });
...
router.get("/items/:id/movements", requirePermission("view_stock"), async (req: AuthRequest, res) => { if (!(await enabled())) return gate(res); const items = await prisma.stockLedgerEntry.findMany({ where: { movement: { stockItemId: req.params.id } }, include: { movement: true }, orderBy: { createdAt: "desc" } }); return res.json({ items, page: 1, pageSize: items.length, total: items.length }); });
...
router.post("/count-sessions/:id/approve", requirePermission("approve_stock_adjustments"), async (req: AuthRequest, res) => {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.stockCountSession.updateMany({ where: { id: session.id, status: "submitted", expectedUpdatedAt: expected }, data: { status: "approved", approvedById: req.user!.id, approvedAt: new Date(), approvalIdempotencyKey: key } });
      if (updated.count !== 1) throw new Error("STALE_WRITE");
      for (const line of session.lines) {
        if (Number(line.variance) === 0) continue;
        const movement = await tx.stockMovement.create({ data: { stockItemId: line.stockItemId, movementType: "count_correction", quantity: Math.abs(Number(line.variance)), reason, idempotencyKey: `count:${session.id}:${line.id}`, performedById: session.startedById, approvedById: req.user!.id, approvedAt: new Date() } });
...
router.post("/movements/transfer", requirePermission("manage_stock"), async (req: AuthRequest, res) => {
  ...
  if (!stockItemId || !sourceLocationId || !destinationLocationId || sourceLocationId === destinationLocationId || !key || !Number.isFinite(quantity) || quantity <= 0) return res.status(400).json({ error: "Distinct locations, positive quantity, and Idempotency-Key header are required.", code: "INVALID_TRANSFER" });
  ...
  const prior = await prisma.stockMovement.findUnique({ where: { idempotencyKey: key } }); if (prior) return res.json(prior);
  try {
    const movement = await prisma.$transaction(async (tx) => {
      ...
      const created = await tx.stockMovement.create({ data: { stockItemId, movementType: "transfer", quantity, idempotencyKey: key, performedById: req.user!.id, reason: typeof b.reason === "string" ? b.reason.trim() : null } });
